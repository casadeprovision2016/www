...
exports.getMinistries = (0, errorHandler_1.asyncHandler)(async (req, res) => {
    const { page = 1, limit = 10, sort = 'name', order = 'asc', ativo } = req.query;
    let query = supabase
        .from('ministries')
        .select(`
      *,
    ...
    if (error) {
        throw new errorHandler_1.AppError('Erro ao buscar ministérios', 500);
    }
    const response = {
        data: data || [],
        total: count || 0,
