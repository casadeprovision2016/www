...
exports.getCustomReport = exports.getYearlyReport = exports.getMonthlyReport = exports.getDashboardStats = void 0;
...
exports.getDashboardStats = (0, errorHandler_1.asyncHandler)(async (req, res) => {
    ...
    try {
        ...
        const [eventsResult, membersResult, donationsResult, visitorsResult, streamsResult, donationAmountResult] = await Promise.all([
        ...
        throw new errorHandler_1.AppError('Erro ao gerar estatísticas do dashboard', 500);
    }
});
exports.getMonthlyReport = (0, errorHandler_1.asyncHandler)(async (req, res) => {
    const { year, month } = req.query;
    if (!year || !month) {
        throw new errorHandler_1.AppError('Ano e mês são obrigatórios', 400);
    }
    const startDate = new Date(parseInt(year), parseInt(month) - 1, 1);
    const endDate = new Date(parseInt(year), parseInt(month), 0, 23, 59, 59);
    try {
        const [eventsResult, donationsResult, newMembersResult, pastoralVisitsResult] = await Promise.all([
            // Eventos do mês
            supabase
                .from('events')
                .select('*')
        ...
        const report = {
            ...
            donations: {
                ...
                by_type: donationsResult.data?.reduce((acc, d) => {
        ...
        throw new errorHandler_1.AppError('Erro ao gerar relatório mensal', 500);
    }
});
exports.getYearlyReport = (0, errorHandler_1.asyncHandler)(async (req, res) => {
    const { year } = req.query;
    if (!year) {
        throw new errorHandler_1.AppError('Ano é obrigatório', 400);
    }
    const startDate = new Date(parseInt(year), 0, 1);
    const endDate = new Date(parseInt(year), 11, 31, 23, 59, 59);
    try {
        const [eventsResult, donationsResult, membersResult, pastoralVisitsResult] = await Promise.all([
            supabase
                .from('events')
                .select('*')
                .gte('data_inicio', startDate.toISOString())
        ...
        throw new errorHandler_1.AppError('Erro ao gerar relatório anual', 500);
    }
});
exports.getCustomReport = (0, errorHandler_1.asyncHandler)(async (req, res) => {
    const { start_date, end_date, include_events, include_donations, include_members, include_visits } = req.query;
    if (!start_date || !end_date) {
        throw new errorHandler_1.AppError('Datas de início e fim são obrigatórias', 400);
    }
    const report = {
        period: {
            start_date,
    ...
    try {
        ...
        throw new errorHandler_1.AppError('Erro ao gerar relatório customizado', 500);
    }
});
