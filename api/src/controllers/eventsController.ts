...
import { AuthenticatedRequest, CreateEventRequest, EventQueryParams, PaginatedResponse, StatsResponse } from '@shared/types';
...
export const getEvents = asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
  const { page = 1, limit = 10, upcoming, past, month, year, sort = 'data_inicio', order = 'desc' } = req.query as any;
...
export const createEvent = asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
  ...
  await cacheService.invalidate('stats:events*');
...
export const registerForEvent = asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
  ...
  if (existing) {
    throw new AppError('Usuário já inscrito neste evento', 400);
  }
  ...
  if (error) {
    throw new AppError('Erro ao realizar inscrição', 500);
  }
...
export const unregisterFromEvent = asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
  const { id } = req.params;
  const userId = req.user.id;
  ...
  if (error) {
    throw new AppError('Erro ao cancelar inscrição', 500);
  }
