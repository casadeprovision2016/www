...
-- ÍNDICES DE PERFORMANCE (VERSÃO SEGURA)
-- =====================================================
-- Arquivo: 02_indices_performance_safe.sql
-- Descrição: Criação de índices para otimizar performance das consultas (sem pg_trgm)
-- Autor: Sistema CCCP
-- Data: 2025-01-24
-- =====================================================
...
-- =====================================================
-- Tentar instalar pg_trgm (pode falhar em alguns ambientes)
DO $$
BEGIN
    CREATE EXTENSION IF NOT EXISTS pg_trgm;
    RAISE NOTICE 'Extensão pg_trgm instalada com sucesso';
EXCEPTION WHEN OTHERS THEN
    RAISE NOTICE 'Aviso: pg_trgm não pôde ser instalada. Índices de similaridade serão ignorados.';
END
$$;

-- =====================================================
-- ÍNDICES BÁSICOS PARA TABELA USERS
-- =====================================================
...
-- Índice de similaridade (só se pg_trgm estiver disponível)
DO $$
BEGIN
    CREATE INDEX IF NOT EXISTS idx_users_nome_trgm ON public.users USING gin(nome gin_trgm_ops);
    RAISE NOTICE 'Índice de similaridade para users.nome criado';
EXCEPTION WHEN OTHERS THEN
    RAISE NOTICE 'Aviso: Índice de similaridade para users.nome ignorado';
END
$$;
...
-- Índice de similaridade para eventos (opcional)
DO $$
BEGIN
    CREATE INDEX IF NOT EXISTS idx_events_titulo_trgm ON public.events USING gin(titulo gin_trgm_ops);
    RAISE NOTICE 'Índice de similaridade para events.titulo criado';
EXCEPTION WHEN OTHERS THEN
    RAISE NOTICE 'Aviso: Índice de similaridade para events.titulo ignorado';
END
$$;
...
-- Índice de similaridade para ministérios (opcional)
DO $$
BEGIN
    CREATE INDEX IF NOT EXISTS idx_ministries_nome_trgm ON public.ministries USING gin(nome gin_trgm_ops);
    RAISE NOTICE 'Índice de similaridade para ministries.nome criado';
EXCEPTION WHEN OTHERS THEN
    RAISE NOTICE 'Aviso: Índice de similaridade para ministries.nome ignorado';
END
$$;
...
BEGIN
    ...
    RAISE NOTICE 'Estatísticas das tabelas atualizadas com sucesso';
EXCEPTION WHEN OTHERS THEN
    RAISE NOTICE 'Aviso: Algumas tabelas podem não ter sido analisadas';
END
$$;
...
BEGIN
    ...
    -- Verificar se pg_trgm existe
    SELECT EXISTS(SELECT 1 FROM pg_extension WHERE extname = 'pg_trgm') INTO extension_exists;
    
    RAISE NOTICE '=====================================================';
    RAISE NOTICE 'RELATÓRIO FINAL DOS ÍNDICES';
    RAISE NOTICE '=====================================================';
    RAISE NOTICE 'Total de índices criados: %', index_count;
    RAISE NOTICE 'Extensão pg_trgm disponível: %', CASE WHEN extension_exists THEN 'SIM' ELSE 'NÃO' END;
    RAISE NOTICE 'Status: CONCLUÍDO COM SUCESSO';
    RAISE NOTICE '=====================================================';
END
$$;
